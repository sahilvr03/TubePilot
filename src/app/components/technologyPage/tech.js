...
export default function TechnologyPage() {
  ...
  const renderSection = (title, plans) => (
    <section className="py-8 px-4 sm:px-6 lg:px-8  text-gray-900">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h2 className={`${orbitron.className} text-3xl sm:text-4xl font-extrabold mb-4 text-cyan-600 drop-shadow-lg`}>
            {title}
          </h2>
          <div className="w-24 h-1 bg-gradient-to-r from-cyan-500 to-pink-500 mx-auto mb-6 rounded-full" />
          <p className={`${oxanium.className} text-lg text-gray-600 max-w-3xl mx-auto`}>
  ...
  const renderSalesSection = () => (
    <section className="py-20 px-4 sm:px-6 lg:px-8  text-gray-900 relative overflow-hidden">
      <div className="max-w-5xl mx-auto text-center">
        <motion.h2
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.8 }}
          className={`${orbitron.className} text-2xl font-extrabold mb-6 text-pink-600 drop-shadow-lg`}
        >
          🚀 Limited-Time Offer!
        </motion.h2>
        ...
        <div className="flex justify-center gap-6 ">
          {["hours", "minutes", "seconds"].map((unit) => (
            <div key={unit} className="text-center bg-white p-4 rounded-xl shadow-sm">
              <p className="text-2xl font-bold text-cyan-600">{timeLeft[unit]}</p>
              <span className="uppercase text-sm text-gray-600">{unit}</span>
            </div>
          ))}
        </div>
